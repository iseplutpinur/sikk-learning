...
$(function () {
    ...
    function uploadImage(image, id) {
        ...
        $.ajax({
            url: "<?= base_url()?>about/kebijakan/uploadImage",
            cache: false,
            contentType: false,
            processData: false,
            data: data,
            ...
            success: function (data) {
                if (data.url.status == 1) {
                    ...
                    if (id.prop("id") == "kebijakan-deskripsi") {
                        profile_image.add(path);
                    } else {
                        kebijakan_image.add(path);
                    }
                } else {
    ...
    function deleteFile(name, id) {
        $.LoadingOverlay("show");
        $.ajax({
            url: "<?= base_url()?>about/kebijakan/deleteImage",
            data: {
                name: name
            },
            type: "post",
    ...
    // simpan kebijakan
    $("#form-kebijakan").submit(function (ev) {
        ev.preventDefault();
        $.LoadingOverlay("show");
        $.ajax({
            url: "<?= base_url()?>about/kebijakan/insert",
            data: {
                ...
                judul_7:            $("#judul-7").val(),
                deskripsi_7:        $("#deskripsi-7").summernote('code'),
                gambar: Array.from(kebijakan_image)
            },
            type: "post",
            success: function (data) {
                Toast.fire({
